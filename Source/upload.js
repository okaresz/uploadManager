	...
	var store = 'umo',
		...
		uploadManager = window.uploadManager = {
			...
			queue: {},
   			...
   			//Counts how many files have benn added. Not the current file count! (If the user cancels an upload, this number isn't decreasing.
   			//used for indexing generated form input fileds uniquely
   			fileCounter: 0,
			...
			postArrayName: 'uploadManagerFiles',
   			...
   			/*
				...
				prepare and check the form for uploadManager on every added upload
   				(needs options to know in which form is the current instance)
			*/

   			prepareForm: function (options) {

				var form = $(options.container).getParent('form');

				if(form)
				{
					if( !form.getElement('input#uploadManager_postArrayName') )
					{
						new Element( 'input', {
							type: 'hidden',
							id: 'uploadManager_postArrayName',
							name: 'uploadManager_postArrayName',
	   						value: options.postArrayName }
						).inject( form, 'top' );
					}
					else
					{
						form.getElement('input#uploadManager_postArrayName').set('value', options.postArrayName );
					}
				}
			},
			...
			*/
			...
			upload: function(options) {
				...
				var opt = Object[append]({fileCounter: this.fileCounter, postArrayName: this.postArrayName, limit: 0, filesize: 0, maxsize: 0, progressbar: true/*, resume: false, iframe: false */}, options),
					container = opt.container,
					transfer;

				this.prepareForm(opt);
		...
		Transfert = new Class({
			...
			createElement: function (options) {
				...
				this[element] = new Element('div', {'class': 'upload-container',
								html: '<iframe id="' + options.id + '_iframe" src="' + options.base + ( options.base.indexOf('?') == -1 ? '?' : '&') + options.id + '" frameborder="no" scrolling="no" style="border:0;overflow:hidden;padding:0;display:block;float:left;height:24px; "></iframe>'
								+ '<input type="checkbox" disabled="disabled" style="display:none" name="'+options.postArrayName+'[' + options.name + ']['+options.fileCounter+'][name]" id="' + options.id + '"/>'
								+ '<input type="checkbox" disabled="disabled" style="display:none" name="'+options.postArrayName+'[' + options.name + ']['+options.fileCounter+'][name]" id="'+ options.id + '_lfile"/>'
		...
		HTML5Transfert = new Class(Object[append]({
			...
			createHTML: function (options) {
				...
				this[element] = new Element('div', {
						...
						+ '<input type="checkbox" disabled="disabled" style="display:none" name="'+options.postArrayName+'[' + options.name + ']['+options.fileCounter+'][name]" id="' + options.id + '"/>'
						+ '<input type="checkbox" disabled="disabled" style="display:none" name="'+options.postArrayName+'[' + options.name + ']['+options.fileCounter+'][file]" id="'+ options.id + '_lfile"/>'
		...
		HTML5MultipartTransfert = new Class(Object[append]({
			...
			createHTML: function (options) {
				...
				this[element] = new Element('div', {
						...
						+ '<input type="checkbox" disabled="disabled" style="display:none" name="'+options.postArrayName+'[' + options.name + ']['+options.fileCounter+'][name]" id="' + options.id + '"/>'
						+ '<input type="checkbox" disabled="disabled" style="display:none" name="'+options.postArrayName+'[' + options.name + ']['+options.fileCounter+'][file]" id="'+ options.id + '_lfile"/>'
						+ '<input type="checkbox" style="display:none" name="'+options.postArrayName+'[' + options.name + ']['+options.fileCounter+'][guid]" id="'+ options.id + '_gfile" checked="checked"/>'
						+ '<label for="'+ options.id + '"></label>'
						...
						load: function () {
							...
							if (status >= 200 && status < 300) {
								...
								try {
									...
									this.guid = json.guid;
									this[element][getElement]('input[name$="[guid]"]').value = json.guid;
									this.upload()
								}
								catch(e) {
